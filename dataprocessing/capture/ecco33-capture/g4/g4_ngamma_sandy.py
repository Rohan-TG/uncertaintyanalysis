...
perturbation_coefficients = np.arange(-0.084, 0.501, 0.001)
...
lower_bound = 2.2313020000e6  # group 4 in eV
upper_bound = 3.6787940000e6   # group 3 in eV
...
for coeff in tqdm.tqdm(perturbation_coefficients, total=len(perturbation_coefficients)):
    ...
    savefilename = f"ECCO33-g4_Pu9_{coeff:0.3f}_MT102.09c"
    with open(f"{savefilename}", mode="w") as f:
        f.write(outs["ace"])

    # savefilependf = f"ECCO33-g4_Pu9_{coeff:0.3f}_MT102.pendf"
