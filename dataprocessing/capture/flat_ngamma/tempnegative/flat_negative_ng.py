...
perturbation_coefficients = np.arange(-0.011, 0.000, 0.001)
...
for coeff in tqdm.tqdm(perturbation_coefficients, total=len(perturbation_coefficients)):
    ...
    savefilename = f"flat_Pu9_{coeff:0.3f}_MT102.09c"
    with open(f"{savefilename}", mode="w") as f:
        f.write(outs["ace"])

    # savefilependf = f"flat_Pu9_{coeff:0.3f}_MT102.pendf"
