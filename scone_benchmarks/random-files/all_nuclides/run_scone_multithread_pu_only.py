...
num_jobs = int(input("Num. parallel jobs: "))

num_cores = int(input('Num. cores per job: '))
...
for i in range(num_jobs):
	subprocess.run(f"mkdir job_{i+1}", shell=True)

	subprocess.run(f"cp {target_path}/{default_xsfile} job_{i+1}/{libfilename}", shell=True)
	subprocess.run(f"cp {target_path}/{default_Jezebel} job_{i+1}/Jezebel", shell=True)
